...
class Worker(QObject):
    ...
    def run(self):
        ...
        def callbackFunc(params, iter, resid, *args, **kws):
            console.debug(f"Iteration: {iter:5d},   Reduced chi2 per {self._proxy.fitting._pointsCount} points: {self._proxy.fitting._chiSq/self._proxy.fitting._pointsCount:10.2f}")
            if self._needCancel:
                self._needCancel = False
